...
**Що калібрує:** компенсує відхилення AD8302 midpoint від ідеальних 0.9 V (допуски компонентів, різна довжина кабелів між антенами, паразитні ємності плати). Записує реальне значення VPHS при точно нульовому куті — "ось як виглядає 0°".

**Вимога:** сигнал цілі має бути присутній під час калібрування (не no-signal baseline).

**Процедура:**
1. Включити RC передавач цілі, розмістити на відомій відстані
2. Вирівняти дрон так щоб ліва і права антени були симетричні відносно цілі (азимут 0°)
3. Утримувати кнопку 3 с в стані `IDLE` або `SEARCHING`
4. ESP32 усереднює VPHS₁ за 100 мс → записує в NVS як `SNT_VPHS_ZERO`
5. Якщо `SENTINEL_ELEVATION_ENABLED=1` — аналогічно для VPHS₂ → `SNT_VPHS_ZERO2`
6. SSD1306 показує: `CAL OK  AZ:912mV`
